...
encoder.fit(['journalist', 'celebrity', 'politician'])
df['Profesion'] = encoder.transform(df['Profesion'])

encoder.fit(['left', 'right'])
df['Bi-Ideologia'] = encoder.transform(df['Bi-Ideologia'])

encoder.fit(['left', 'moderate_left', 'moderate_right', 'right'])
...
df.to_csv('df-train_corpus_normalized.csv', index=False)